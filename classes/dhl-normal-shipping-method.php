...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	function dhl_normal_shipping_init() {
		if ( ! class_exists( 'WC_DHL_Normal_Shipping_Method' ) ) {
			class WC_DHL_Normal_Shipping_Method extends WC_Shipping_Method {

				const SHIPPING_METHOD = "Normal Shipping";
				...
				public function __construct( $instance_id = 0) {
                    $this->id                 = 'dhl_normal_shipping_method'; // Id for your shipping method. Should be unique.
                    $this->instance_id        = absint($instance_id);
                    $this->domain             = 'dhl_ns';
					$this->method_title       = __( 'DHL Parcel - Home Delivery' , $this->domain);  // Title shown in admin
					$this->method_description = __( 'Enables home delivery by the DHL services' , $this->domain); // Description shown in admin
                    $this->supports = array(
                        'shipping-zones',
                        'instance-settings',
                ...
                */
                function init_form_fields() {
                    $this->instance_form_fields = array(
                        'title' => array(
                            ...
                            'default'       => __( 'DHL Home Delivery ', $this->domain ),
						),
						'max_weigth' => array(
                            'type'          => 'number',
				...
				function deleted_rules( $new, $old ) {
					...
					$old_rules = $dbh->get_shipping_rules_by_zone_id($zone->get_id(), $vars['id']);
    ...
    add_action( 'woocommerce_shipping_init', 'dhl_normal_shipping_init' );
    
	function add_dhl_normal_shipping_method( $methods ) {
		$methods['dhl_normal_shipping_method'] = 'WC_DHL_Normal_Shipping_Method';
		return $methods;
	}
	add_filter( 'woocommerce_shipping_methods', 'add_dhl_normal_shipping_method' );
